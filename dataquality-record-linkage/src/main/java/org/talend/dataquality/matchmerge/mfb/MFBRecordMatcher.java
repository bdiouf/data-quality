...
public class MFBRecordMatcher extends AbstractRecordMatcher {
    ...
    private static double matchScore(Attribute leftAttribute, Attribute rightAttribute, IAttributeMatcher matcher) {
        ...
        Iterator<String> leftValues = new IteratorChain(Collections.singleton(left).iterator(), leftAttribute.getValues()
                .iterator());
        double maxScore = 0;
        while (leftValues.hasNext()) {
            String leftValue = leftValues.next();
            Iterator<String> rightValues = new IteratorChain(Collections.singleton(right).iterator(), rightAttribute.getValues()
                    .iterator());
