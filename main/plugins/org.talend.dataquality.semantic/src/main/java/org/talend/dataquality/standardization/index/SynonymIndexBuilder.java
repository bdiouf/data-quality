...
public class SynonymIndexBuilder {
    ...
    public void initIndexInFS(String path) {
        ...
        try {
            ...
            log.error("SynonymIndexBuilder.failLoad");//$NON-NLS-1$
            // e.printStackTrace();
        }
    }
    ...
    public boolean insertDocument(String word, String synonyms) throws IOException {
        if (word.length() == 0) {
            log.error("SynonymIndexBuilder.noRef");//$NON-NLS-1$
            return false;
        }
    ...
    public boolean insertDocumentIfNotExists(String word, String synonyms) throws IOException {
        ...
        log.error("SynonymIndexBuilder.aDocument" + word);//$NON-NLS-1$
        return false;
    }
    ...
    public int updateDocument(String word, String synonyms) throws IOException {
        ...
        default:
            ...
            log.error("SynonymIndexBuilder.documents " + docs.totalHits + " " + word);//$NON-NLS-1$
            break;
        }
    ...
    public int deleteDocumentByWord(String word) throws IOException {
        ...
        switch (docs.totalHits) {
        case 0:
            log.error("SynonymIndexBuilder.doesnotExsit " + word);//$NON-NLS-1$
            return 0;
        case 1:
    ...
    public int addSynonymToDocument(String word, String newSynonym) throws IOException {
        ...
        switch (docs.totalHits) {
        case 0:
            log.error("SynonymIndexBuilder.document " + word);//$NON-NLS-1$
            break;
    ...
    public int removeSynonymFromDocument(String word, String synonymToDelete) throws IOException {
        if (synonymToDelete == null) {
            log.error("SynonymIndexBuilder.theSynonym " + word);//$NON-NLS-1$
            return 0;
        }
        String tempSynonymToDelete = synonymToDelete.trim();
        if (tempSynonymToDelete.toLowerCase().equals(word.toLowerCase())) {
            log.error("SynonymIndexBuilder.synonymToDelete " + tempSynonymToDelete + " " + word);//$NON-NLS-1$
            return 0;
        }
        int deleted = 0;
        ...
        switch (docs.totalHits) {
        case 0:
            log.error("SynonymIndexBuilder.documentNotExsit " + word);//$NON-NLS-1$
            deleted = 0;
            break;
        case 1:
            ...
            if (deleted == 0) {
                log.error("SynonymIndexBuilder.synonymNotExsit " + tempSynonymToDelete);//$NON-NLS-1$
            } else {
                doc = generateDocument(doc.getValues(SynonymIndexSearcher.F_WORD)[0], synonymList);
    ...
    public boolean deleteIndexFromFS(String path) {
        ...
        if (folder.isDirectory()) {
            ...
            if (filelist.length == 0) {// folder is empty
                if (!folder.delete()) {
                    log.error("SynonymIndexBuilder.couldNotDelete " + folder.getAbsolutePath());//$NON-NLS-1$
                    return false;
                }
            } else {
                Status status = null;
                ...
                if (status == null || status.missingSegments) {
                    log.error("SynonymIndexBuilder.notAnIndexFolder " + folder.getAbsolutePath());//$NON-NLS-1$
                    return false;
            ...
            log.error("SynonymIndexBuilder.pathIsFile " + folder.getAbsolutePath());//$NON-NLS-1$
            return false;
        }
        return true;
    }
    ...
    private IndexSearcher getNewIndexSearcher() throws IOException {
        ...
        if (status.missingSegments) {
            System.err.println("SynonymIndexBuilder.print");//$NON-NLS-1$
        }
