...
/**
 * Analyze the quantiles given t-digest algorithm implemented by clearspring's
 * "stream-lib" package. See more at <a href=
 ...
 *
 */
public class TDigestAnalyzer extends
		NumericalStatisticsAnalyzer<TDigestQuantileStatistics> {

    private static final long serialVersionUID = -9176043422228459277L;

    private final ResizableList<TDigestQuantileStatistics> stats = new ResizableList<>(
			TDigestQuantileStatistics.class);
		...
		if (record.length != types.length)
			throw new IllegalArgumentException(
					"Each column of the record should be declared a DataType.Type corresponding! \n"
							+ types.length
							+ " type(s) declared in this T-Digest analyzer but "
							+ record.length
